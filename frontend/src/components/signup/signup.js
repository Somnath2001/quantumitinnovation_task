...
import "./signup.css";
import { authenticate, isAuthenticated, signup } from "../../apis/helper";
...
const Signup = () => {
  ...
  const { name, email, birthDate, password, error, success, loading } = values;
  ...
  const onSubmit = (event) => {
    ...
    console.log(values);
    signup({ name, birthDate, email, password }).then((data) => {
      console.log(data);
      if (data.error) {
        setValues({
          ...values,
  ...
  return (
    <div>
      <Background />
      <div className="signup_outerLayout">
        <div className="signup_auth_card">
          <svg
            width="52"
            height="48"
            viewBox="0 0 52 48"
